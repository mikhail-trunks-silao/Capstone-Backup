...
function heuristic(node, end, transfers) {
  const transferCost = 10000;
  const dist = distance(node[0], node[1], end[0], end[1]);
  return dist + transfers * transferCost;
}
...
function aStar(graph, start, end) {
  ...
  graph.forEach((_, coord) => {
    gScore.set(coord, Infinity);
    fScore.set(coord, Infinity);
  });

  gScore.set(`${start[0]},${start[1]}`, 0);
  fScore.set(`${start[0]},${start[1]}`, heuristic(start, end, 0));

  while (!openSet.isEmpty()) {
    const current = openSet.get();
    const currentKey = `${current[0]},${current[1]}`;

    if (currentKey === `${end[0]},${end[1]}`) {
    ...
    for (const neighbor of graph.get(currentKey)) {
      ...
      if (tentativeGScore < gScore.get(neighborKey)) {
        ...
        const transfers = neighbor.routeName !== null ? 1 : 0;
        fScore.set(neighborKey, tentativeGScore + heuristic(neighbor.coord, end, transfers));

        if (!openSet.elements.find(item => item.item[0] === neighbor.coord[0] && item.item[1] === neighbor.coord[1])) {
          openSet.put(neighbor.coord, fScore.get(neighborKey));
        }
      }
    }
  }
...
export function findShortestPath(jeepneyRoutes, currentLocation, destination, walkingDistanceThreshold = 20) {
  ...
  const path = findShortestPathForStartingPoint(jeepneyRoutes, currentLocation, destination, walkingDistanceThreshold);
  allPaths.push(path);
