...
function preprocessRoutes(jeepneyRoutes, maxDistance = 5) {
...
function findShortestPath(jeepneyRoutes, currentLocation, destination, walkingDistanceThreshold = 20) {
  ...
  for (const [routeName, route] of Object.entries(jeepneyRoutes)) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      const start = route[i];
      const end = route[i + 1];
      const weight = .1; // Set weight to 1 to minimize the number of routes ridden
      ...
      for (const coord1 of route1) {
        for (const coord2 of route2) {
          if (isWithin20Meters(coord1, coord2)) {
            const weight = .5; // Set weight to 1 to minimize the number of routes ridden
