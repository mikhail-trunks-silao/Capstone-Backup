...
function preprocessRoutes(jeepneyRoutes, maxDistance = 20) {
...
function  findShortestPathWithFixedStartPoint(jeepneyRoutes, currentLocation, destination, walkingDistanceThreshold = 20) {
  const graph = new Map();
  ...
  const startPoint = findNearestStartPoint(jeepneyRoutes, currentLocation);
...
function findShortestPath(jeepneyRoutes, currentLocation, destination, walkingDistanceThreshold = 20) {
  const startPointCandidates = findAllStartPointsWithin30Meters(jeepneyRoutes, currentLocation);
  let bestPath = null;
  let bestPathLength = Infinity;

  for (const startPoint of startPointCandidates) {
    const path = findShortestPathWithFixedStartPoint(jeepneyRoutes, startPoint, destination, walkingDistanceThreshold);
...
function findAllStartPointsWithin30Meters(jeepneyRoutes, userLocation) {
  let startPoints = [];
...
function computePathLength(path) {
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (Array.isArray(prev) && Array.isArray(curr)) {
      length += distance(prev[1][0], prev[1][1], curr[1][0], curr[1][1]);
    }
  }
